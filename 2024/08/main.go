...
func main() {
	...
	numberOfAntinodeWithResonanceHarmonics := calculateNumberOfAntinodeWithResonanceHarmonics(antennaCoordinateMap, maxRow, maxColumn)
	fmt.Printf("number of antinode with resonance harmonics: %d\n", numberOfAntinodeWithResonanceHarmonics)
}
...
func calculateNumberOfAntinodeWithResonanceHarmonics(antennaCoordinateMap map[string][][]int, maxRow, maxColumn int) int {
	numberOfAntinode := 0
	...
	for _, coordinates := range antennaCoordinateMap {
		for i, firstCoord := range coordinates {
			for j := i + 1; j < len(coordinates); j++ {
				...
				firstCoordError := false
				secondCoordError := false
				for k := 1; ; k++ {
					if !firstCoordError {
						firstAntinodeCoord := []int{firstCoord[0] + (k * rowDist), firstCoord[1] + (k * columnDist)}
						if isCoordinateInsideMap(firstAntinodeCoord, maxRow, maxColumn) {
							addUniqueNumberOfAntinodes(firstAntinodeCoord)
						} else {
							firstCoordError = true
						}
					}

					if !secondCoordError {
						secondAntinodeCoord := []int{secondCoord[0] - (k * rowDist), secondCoord[1] - (k * columnDist)}
						if isCoordinateInsideMap(secondAntinodeCoord, maxRow, maxColumn) {
							addUniqueNumberOfAntinodes(secondAntinodeCoord)
						} else {
							secondCoordError = true
						}
					}

					if firstCoordError && secondCoordError {
						break
					}
				}

				addUniqueNumberOfAntinodes(secondCoord)
			}
			addUniqueNumberOfAntinodes(firstCoord)
		}
	}
